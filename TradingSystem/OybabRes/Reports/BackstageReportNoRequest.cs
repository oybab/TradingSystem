...
namespace Oybab.Reports
{
    internal sealed partial class BackstageReportNoRequest : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void xrcProductName_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (lang == 0)
                ((XRTableCell)sender).Text = product.ProductNameZH;
            else if (lang == 1)
                ((XRTableCell)sender).Text = product.ProductNameUG;
            else if (lang == 2)
                ((XRTableCell)sender).Text = product.ProductNameEN;
