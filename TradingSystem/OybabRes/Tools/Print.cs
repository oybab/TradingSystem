...
using static Oybab.Res.Resources;
using Oybab.Report;
...
namespace Oybab.Res.Tools
{
    public sealed class Print
    {
        ...
        public void PrintBarcode(List<ProductLabel> models, int sizeMode, int langIndex)
        {
            ...
            {
                report = new ProductLabelMedium(Resources.GetRes().PrintInfo.PriceSymbol);
            }
            else if (sizeMode == 1) //3cmX2cm
            {
                report = new ProductLabelSmall(Resources.GetRes().PrintInfo.PriceSymbol);
            }
            ...
            ReportProcess.Instance.PrintReport(report, reportModel, null);
        ...
        /// <param name="Lang"></param>
        /// <returns></returns>
        public bool PrintSummary(SummaryModelPackage package, long Lang = -1)
        {
            if (Resources.GetRes().IsLocalPrintCustomOrder)
            {
                ...
                return OperatesService.GetOperates().ServicePrint(JsonConvert.SerializeObject(package), 24, Lang, 1);
        }
        ...
        public bool PrintOrder(Order model, long Lang)
        {
            if (Resources.GetRes().IsLocalPrintCustomOrder)
            {
                PrintOrderAfterCheckout(model, null == model.tb_orderdetail ? null : model.tb_orderdetail.ToList(), Lang);
                return true;
            }
            else
        ...
        /// <param name="Lang"></param>
        /// <returns></returns>
        public bool PrintTakeout(Takeout model, long Lang)
        {
            if (Resources.GetRes().IsLocalPrintCustomOrder)
            {
                PrintTakeoutAfterCheckout(model, null == model.tb_takeoutdetail ? null : model.tb_takeoutdetail.ToList(), Lang);
                return true;
            }
            else
                return OperatesService.GetOperates().ServicePrint(JsonConvert.SerializeObject(model), 13, Lang);
        }
        ...
        {
            // 如果是本地打印账单才继续
            if (Resources.GetRes().PrintInfo.IsPrintBillAfterBuy && Resources.GetRes().IsLocalPrintCustomOrder)
            {
        ...
        {
            // 如果是本地打印账单才继续
            if ((Resources.GetRes().PrintInfo.IsPrintBillAfterCheckout || Lang != -1)  && Resources.GetRes().IsLocalPrintCustomOrder)
            {
                Task.Factory.StartNew(() =>
                {
                    try
        ...
        /// <param name="orderDetails"></param>
        /// <param name="Lang"></param>
        public void PrintTakeoutAfterCheckout(Takeout takeout, List<TakeoutDetail> takeoutDetails, long Lang = -1)
        {
            // 如果是本地打印账单才继续
            if ((Resources.GetRes().PrintInfo.IsPrintBillAfterCheckout || Lang != -1) && Resources.GetRes().IsLocalPrintCustomOrder)
            {
                Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        // 打印客户账单
                        // 开始循环打印客户账单部分
                        Dictionary<Printer, List<TakeoutDetail>> printTakeoutDetails = printDetails.Where(x => x.Key.PrintType == 0  || x.Key.PrintType == 2).ToDictionary(x => x.Key, x => x.Value);
                        PrintCustomerTakeoutAfterCheckout(printTakeoutDetails, takeout, Lang);



                        // 后台打印只在服务器进行, 所以不在这里写(本身我认为打印都应该统一有一个服务器发送, 但是考虑到有些钱箱和打印机在一起, 所以才出了个本地打印功能. 不然也没必要费那个劲)
                            ...
                            if (Lang != -1)
                                ...
                                if (null != import.SupplierId && import.SupplierId != 0)
                                {
                                    if (null != import.tb_supplier)
                                    {
                                        ...
                                        reportModel.Parameters.Add("SupplierIdValue", import.tb_supplier.SupplierNo);
                                    }
                                }
        ...
        internal void PrintSummary(SummaryModelPackage Package)
        {
            // 如果是本地打印账单才继续
            if (Resources.GetRes().IsLocalPrintCustomOrder)
            {
                    ...
                    try
                    {
                        ...
                        foreach (var item in printDetails)
                        {
                            try
                            {
                                ...
                                //reportModel.Parameters.Add("UnrecordTotalPriceValue", Resources.GetRes().RoomsModel.Where(x => null != x.PayOrder).Sum(x => x.PayOrder.TotalPrice));
                                //reportModel.Parameters.Add("UnrecordReceivePrice", Resources.GetRes().GetString("ReceivedPrice", ci));
                                //reportModel.Parameters.Add("UnrecordReceivePriceValue", Resources.GetRes().RoomsModel.Where(x => null != x.PayOrder).Sum(x => x.PayOrder.TotalPaidPrice));
        ...
        private void PrintCustomerOrderAfterBuy(Dictionary<Printer, List<OrderDetail>> printDetails, Order order, Order oldOrder)
        {
            ...
            foreach (var item in printDetails)
            {
                PrintOrderAfterBuyProcess(item.Key, item.Value, order, oldOrder, lastPrinter, ref IsSuccessProcessRoomPrice);
            }

            if (!IsSuccessProcessRoomPrice && null != printDetails && printDetails.Count == 0)
            {
                PrintOrderAfterBuyProcess(lastPrinter, null, order, oldOrder, lastPrinter, ref IsSuccessProcessRoomPrice);
            }
        }
        ...
        /// <param name="order"></param>
        /// <param name="Lang"></param>
        private void PrintCustomerTakeoutAfterCheckout(Dictionary<Printer, List<TakeoutDetail>> printDetails, Takeout takeout, long Lang = -1)
        {
                ...
                try
                {
                    ...
                    if (null != takeout.MemberId && takeout.MemberId != 0)
                    {
                        if (null != takeout.tb_member)
                        {
                            ...
                            reportModel.Parameters.Add("MemberIdValue", takeout.tb_member.MemberNo);
                        }
                    }
        ...
        private void PrintOrderToBackstageProcess(Printer printer, List<OrderDetail> items, Order order)
        {
            try
            {
                ...
                reportModel.Parameters.Add("RoomNoValue", string.Format(": {0}", Resources.GetRes().RoomsModel.Where(x => x.RoomId == order.RoomId).FirstOrDefault().RoomNo));
                reportModel.Parameters.Add("Time", Resources.GetRes().GetString("Time", ci));
        ...
        private void PrintOrderAfterBuyProcess(Printer printer, List<OrderDetail> items, Order order, Order oldOrder, Printer lastPrinter, ref bool IsSuccessProcessRoomPrice)
        {
            try
            {
                object report = null;
                ...
                Room room = Resources.GetRes().RoomsModel.Where(x => x.RoomId == order.RoomId).FirstOrDefault();
        ...
        private void PrintOrderAfterCheckoutProcess(Printer printer, List<OrderDetail> items, Order order, Printer lastPrinter, long Lang, ref bool IsSuccessProcessRoomPrice)
        {
            ...
            try
            {
                ...
                if (printer.PrintType == 0)
                    ...
                    report = new OrderCheckoutMiddleReport(order, Resources.GetRes().Products, printer, Resources.GetRes().PrintInfo.PageHeight, Resources.GetRes().PrintInfo.PriceSymbol, Lang);
                ...
                if (null != order.MemberId && order.MemberId != 0)
                {
                    ...
                    {
                        reportModel.Parameters.Add("MemberId", Resources.GetRes().GetString("MemberNo", ci));
                        reportModel.Parameters.Add("MemberIdValue", order.tb_member.MemberNo);
                    }
                }
