...
{
    internal sealed partial class BalancePayWindow : KryptonForm
    {
        //页数变量
        private int ListCount = 50;
        private int CurrentPage = 1;
        private int AllPage = 1;
        private List<BalancePay> resultList = null;
        private List<CommonPayModel> commonPayList = null;
        private TimeSpan TimeLimit = TimeSpan.FromDays(Resources.GetRes().ShorDay);
        private Balance balance;
        public Balance ReturnValue { get; private set; } //返回值

        public BalancePayWindow(Balance balance)
        {
            InitializeComponent();
            ...
            this.Text = Resources.GetRes().GetString("BalancePay") + " - " + GetBalanceType(new CommonPayModel() { BalanceId = balance.BalanceId });
            ...
            this.Icon = new Icon(asm.GetManifestResourceStream(@"Oybab.Res.Resources.Images.PC.BalancePay.ico"));

            //初始化
            Init();

            // 防止没搜索就增加完时加入时报错
            this.resultList = new List<BalancePay>();
        ...
        private void Init()
        {
            krpcmBalancePayId.HeaderText = Resources.GetRes().GetString("Id");
        ...
        private void OpenPageTo(int pageNo, bool Manual = true)
        {
            ...
            {
                AddToGrid("", item.PayId.ToString(), item.Price, GetBalanceType(item), item.Remark, item.AddTime, item);
            }
        }
        ...
        private void AddToGrid(string editMark, string Id, double Price, string BalanceName, string Remark, long AddTime, CommonPayModel model)
        {
            ...
            if (editMark == "*")
            {
                ...
                krpdgList.Rows[0].Cells["krpcmBalanceType"].ReadOnly = true;
            }
            else
            {
        ...
        private bool IgnoreOperateForSave()
        {
            ...
            if (notHandle)
            {
                var confirm = KryptonMessageBox.Show(this, Resources.GetRes().GetString("IgnoreData"), Resources.GetRes().GetString("BalancePay"), MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        {
            OpenPageTo(1);
            AddToGrid("*", "-1", 0, GetBalanceType(new CommonPayModel() { BalanceId = balance.BalanceId }), "", long.Parse(DateTime.Now.ToString("yyyyMMddHHmmss")), null);
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                else
                {
                    ...
                    if (krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*"))
                    {
                        ...
                        // 只有还没保存的才能删除
                        if (krpdgList.SelectedRows[0].Cells["krpcmBalancePayId"].Value.ToString() == "-1")
        ...
        private void krpdgList_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Control && lastKeyPressed != Keys.EraseEof)
            {
                ...
                if (e.KeyCode == Keys.D)
                {
                    ...
                    // 只有还没保存的才能删除
                    if (krpdgList.SelectedRows.Count > 0 && krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*") && krpdgList.SelectedRows[0].Cells["krpcmBalancePayId"].Value.ToString() == "-1")
                        Delete();
                }
            }
            else if (e.Control)
        ...
        internal void ReloadBalances()
        {
            ...
            {
                if (Resources.GetRes().MainLangIndex == 0)
                    krpcmBalanceType.Items.AddRange(Resources.GetRes().Balances.OrderByDescending(x => x.Order).ThenByDescending(x => x.BalanceId).Select(x => x.BalanceName0).ToArray());
                else if (Resources.GetRes().MainLangIndex == 1)
                    krpcmBalanceType.Items.AddRange(Resources.GetRes().Balances.OrderByDescending(x => x.Order).ThenByDescending(x => x.BalanceId).Select(x => x.BalanceName1).ToArray());
                else if (Resources.GetRes().MainLangIndex == 2)
                    krpcmBalanceType.Items.AddRange(Resources.GetRes().Balances.OrderByDescending(x => x.Order).ThenByDescending(x => x.BalanceId).Select(x => x.BalanceName2).ToArray());
            }
        ...
        {
            if (null != model && (model.Type == "ImportPay" ||  model.Type == "SupplierPay" || model.Type == "AdminPay"))
                return -price;
            else
                return price;
        }
        ...
        private string GetOperateName(CommonPayModel model)
        {
            ...
            OperateName = OperateName + ( model.Type == "BalancePay" ? "" : model.ParentId + " - ");

            OperateName += "  ";

            OperateName += Resources.GetRes().PrintInfo.PriceSymbol + model.BalancePrice;
