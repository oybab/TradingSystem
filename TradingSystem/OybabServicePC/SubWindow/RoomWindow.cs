...
{
    internal sealed partial class RoomWindow : KryptonForm
    {
        //页数变量
        ...
        public RoomWindow()
        {
            ...
            Notification.Instance.NotificationRoom += (obj, value, args) => { this.BeginInvoke(new Action(() => { if (krpdgList.Enabled && resultList.Any(x => x.RoomId == value.RoomId)) krpdgList.Enabled = false; })); };

            new CustomTooltip(this.krpdgList);
            this.ControlBox = false;
            
            krptRoomNo.Font = new Font(Resources.GetRes().GetString("FontName2"), float.Parse(Resources.GetRes().GetString("FontSize")));
            this.Text = Resources.GetRes().GetString("RoomManager");
            ResetPage();
            ...
            if (int.Parse(Resources.GetRes().GetString("HightFix")) != 0)
            {
                ...
                krptRoomNo.Location = new Point(krptRoomNo.Location.X, krptRoomNo.Location.Y + (int.Parse(Resources.GetRes().GetString("HightFix")) / 2).RecalcMagnification2());
            ...
            this.Icon = new Icon(asm.GetManifestResourceStream(@"Oybab.Res.Resources.Images.PC.Room.ico"));
            ...
            //防止直接增加数据的时候插入本地队列失败
            resultList = new List<Room>();


            // 点歌系统就显示最低消费
            if (Resources.GetRes().IsRequired("Vod"))
                this.krpcmFreeRoomPriceLimit.Visible = true;
        ...
        private void Init()
        {
            krpcmRoomId.HeaderText = Resources.GetRes().GetString("Id");
            krpcmRoomNo.HeaderText = Resources.GetRes().GetString("RoomNo");
            krpcmPrice.HeaderText = Resources.GetRes().GetString("Price");
            krpcmIsPayByTime.HeaderText = Resources.GetRes().GetString("IsPayByTime");
            krpcmPriceHour.HeaderText = Resources.GetRes().GetString("PriceHour");
            krpcmFreeRoomPriceLimit.HeaderText = Resources.GetRes().GetString("FreeRoomPriceLimit");
            krpcmIsAutoExtendTime.HeaderText = Resources.GetRes().GetString("IsAutoExtendTime");
            ...
            krpcmIsPayByTime.Items.AddRange(new string[] { Resources.GetRes().GetString("No"), Resources.GetRes().GetString("Hour"), Resources.GetRes().GetString("Day") });
            krpcmHideType.Items.AddRange(new string[] { Resources.GetRes().GetString("Display"),  Resources.GetRes().GetString("Hide") });
        ...
        private void krpbSearch_Click(object sender, EventArgs e)
        {
            ...
            //查找数据
            resultList = Resources.GetRes().Rooms.OrderByDescending(x => x.Order).ThenBy(x => x.RoomNo.Length).ThenBy(x => x.RoomNo).ToList();
            if (krptRoomNo.Text.Trim() != "")
                resultList = resultList.Where(x => x.RoomNo.Contains(krptRoomNo.Text.Trim(), StringComparison.OrdinalIgnoreCase)).ToList();
        ...
        private void OpenPageTo(int pageNo, bool Manual = true)
        {
            ...
            {
                AddToGrid("", item.RoomId.ToString(), item.RoomNo, item.Price, GetTimeType(item.IsPayByTime), item.PriceHour, item.FreeRoomPriceLimit, item.IsAutoExtendTime, item.Order, GetHideType(item.HideType));
            }
        }
        ...
        private void AddToGrid(string editMark, string Id, string roomNo, double price, string isPayByTime, double PriceHour, double FreeRoomPriceLimit, long IsAutoExtendTime, long Order, string HideType)
        {
            if (editMark == "*")
                krpdgList.Rows.Insert(0, editMark, Id, roomNo, price.ToString(), isPayByTime, PriceHour.ToString(), FreeRoomPriceLimit.ToString(), IsAutoExtendTime.ToString(), Order.ToString(), HideType);
            else
                krpdgList.Rows.Add(editMark, Id, roomNo, price.ToString(), isPayByTime, PriceHour.ToString(), FreeRoomPriceLimit.ToString(), IsAutoExtendTime.ToString(), Order.ToString(), HideType);
        }
        ...
        private bool IgnoreOperateForSave()
        {
            ...
            if (notHandle)
            {
                var confirm = KryptonMessageBox.Show(this, Resources.GetRes().GetString("IgnoreData"), Resources.GetRes().GetString("RoomManager"), MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void Add(){
            OpenPageTo(1);
            AddToGrid("*", "-1", "", 0, GetTimeType(0), 0, 0, 0, 0, GetHideType(0));
        ...
        private void Save()
        {
            ...
            {
                //如果是插入
                if (krpdgList.SelectedRows[0].Cells["krpcmRoomId"].Value.ToString().Equals("-1"))
                {
                    Room model = new Room();
                    try
                    {
                        model.RoomId = long.Parse(krpdgList.SelectedRows[0].Cells["krpcmRoomId"].Value.ToString());
                        model.RoomNo = krpdgList.SelectedRows[0].Cells["krpcmRoomNo"].Value.ToString().Trim();
                        model.Price = Math.Round(double.Parse(krpdgList.SelectedRows[0].Cells["krpcmPrice"].Value.ToString()), 2);
                        model.IsPayByTime = GetTimeTypeNo(krpdgList.SelectedRows[0].Cells["krpcmIsPayByTime"].Value.ToString());
                        model.PriceHour = Math.Round(double.Parse(krpdgList.SelectedRows[0].Cells["krpcmPriceHour"].Value.ToString()), 2);
                        model.FreeRoomPriceLimit = Math.Round(double.Parse(krpdgList.SelectedRows[0].Cells["krpcmFreeRoomPriceLimit"].Value.ToString()), 2);
                        model.IsAutoExtendTime = long.Parse(krpdgList.SelectedRows[0].Cells["krpcmIsAutoExtendTime"].Value.ToString());
                        ...
                        //判断是否已存在
                        if (Resources.GetRes().Rooms.Where(x => (x.RoomNo.Equals(model.RoomNo, StringComparison.OrdinalIgnoreCase))).Count() > 0)
                        {
                        ...
                        // 不能超出雅座数量限制
                        if (Resources.GetRes().Rooms.Where(x=>x.HideType == 0 || x.HideType == 2).Count() >= Resources.GetRes().RoomCount && (model.HideType == 0 || model.HideType == 2))
                        {
                            KryptonMessageBox.Show(this, string.Format(Resources.GetRes().GetString("Exception_RoomCountOutOfLimit"), Common.GetCommon().GetFormat()), Resources.GetRes().GetString("Warn"), MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            return;
                        }
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        try
                        {
                            string roomOrderSession;
                            bool result = OperatesService.GetOperates().ServiceAddRoom(model, out roomOrderSession);

                            this.BeginInvoke(new Action(() =>
                            {
                                if (result)
                                {
                                    krpdgList.SelectedRows[0].Cells["krpcmRoomId"].Value = model.RoomId;
                                    ...
                                    ReloadRoomNoTextbox(true);



                                    if (model.HideType == 0)
                                    {
                                        if (!string.IsNullOrWhiteSpace(roomOrderSession))
                                        {
                                            Resources.GetRes().RoomsModel.Add(new RoomModel() { Order = model.Order, HideType = model.HideType, RoomId = model.RoomId, RoomNo = model.RoomNo, OrderSession = roomOrderSession });
                                        }
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        model = Resources.GetRes().Rooms.Where(x => x.RoomId == model.RoomId).FirstOrDefault().FastCopy();
                        ...
                        //判断是否已存在
                        if (Resources.GetRes().Rooms.Where(x => x.RoomId != model.RoomId && (x.RoomNo.Equals(model.RoomNo, StringComparison.OrdinalIgnoreCase))).Count() > 0)
                        {
                        ...
                        //如果已经有使用,则提示并拒绝删除
                        if (Resources.GetRes().RoomsModel.Where(x => x.RoomId == model.RoomId && null != x.PayOrder).Count() > 0) // && x.HideType == 0 && model.HideType ==  1 (本来是开启的不能隐藏. 但是现在改为只要是在用, 就不能修改)
                        {
                            KryptonMessageBox.Show(this, Resources.GetRes().GetString("TableUsed"), Resources.GetRes().GetString("Warn"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }

                        // 不能超出雅座数量限制
                        if (Resources.GetRes().Rooms.Where(x => x.HideType == 0 || x.HideType == 2).Count() >= Resources.GetRes().RoomCount && resultList.Where(x=>x.RoomId == model.RoomId).FirstOrDefault().HideType == 1 && (model.HideType == 0 || model.HideType == 2))
                        {
                            KryptonMessageBox.Show(this, string.Format(Resources.GetRes().GetString("Exception_RoomCountOutOfLimit"), Common.GetCommon().GetFormat()), Resources.GetRes().GetString("Warn"), MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            return;
                        }
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        {
                            string roomOrderSession;
                            ResultModel result  = OperatesService.GetOperates().ServiceEditRoom(model, out roomOrderSession);
                            ...
                            this.BeginInvoke(new Action(() =>
                            {
                                if (result.Result)
                                {
                                    KryptonMessageBox.Show(this, Resources.GetRes().GetString("SaveSuccess"), Resources.GetRes().GetString("Information"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value = "";
                                    Room oldModel = resultList.Where(x => x.RoomId == model.RoomId).FirstOrDefault();

                                    int no = resultList.IndexOf(oldModel);
                                    resultList.RemoveAt(no);
                                    resultList.Insert(no, model);

                                    no = Resources.GetRes().Rooms.IndexOf(oldModel);
                                    Resources.GetRes().Rooms.RemoveAt(no);
                                    Resources.GetRes().Rooms.Insert(no, model);

                                   

                                    RoomModel oldRoomModel = Resources.GetRes().RoomsModel.Where(x => x.RoomId == model.RoomId).FirstOrDefault();

                                    if (model.HideType == 1 && oldModel.HideType == 0)
                                    {
                                        if (null != oldRoomModel)
                                            Resources.GetRes().RoomsModel.Remove(oldRoomModel);
                                        RemoveRoom(model, null);
                                    }
                                    else if (model.HideType == 0 && oldModel.HideType == 1)
                                    {
                                        if (null != oldRoomModel)
                                        {
                                            Resources.GetRes().RoomsModel.Remove(oldRoomModel);
                                    ...
                                    else if (null != oldRoomModel && !string.IsNullOrWhiteSpace(roomOrderSession))
                                    {
                                        oldRoomModel.OrderSession = roomOrderSession;

                                        ChangeName(model, null);
                                        ReloadRoomNoTextbox(true);
        ...
        private void Delete()
        {
            ...
            try
            {
                ...
                Id = long.Parse(krpdgList.SelectedRows[0].Cells["krpcmRoomId"].Value.ToString());
                ...
                //如果已经有使用,则提示并拒绝删除
                if (Resources.GetRes().RoomsModel.Where(x => x.RoomId == Id && null != x.PayOrder).Count() > 0)
                {
                    KryptonMessageBox.Show(this, Resources.GetRes().GetString("TableUsed"), Resources.GetRes().GetString("Warn"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    ResultModel result = OperatesService.GetOperates().ServiceDelRoom(Resources.GetRes().Rooms.Where(x => x.RoomId == Id).FirstOrDefault());
                    this.BeginInvoke(new Action(() =>
                    {
                        if (result.Result)
                        {
                            KryptonMessageBox.Show(this, Resources.GetRes().GetString("DeleteSuccess"), Resources.GetRes().GetString("Information"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Room oldModel = Resources.GetRes().Rooms.Where(x => x.RoomId == Id).FirstOrDefault();
                            krpdgList.Rows.Remove(krpdgList.SelectedRows[0]);
                            resultList.Remove(oldModel);
                            Resources.GetRes().Rooms.Remove(oldModel);
                            ReloadRoomNoTextbox(true);
                            RemoveRoom(oldModel, null);
                        }
                        else
                        {
                            if (result.IsDataHasRefrence)
        ...
        private void krpdgList_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                Oybab.ServicePC.Tools.BakInOperate.Instance.Output(this, new BakInModel() { Rooms = resultList });
            }
            // 导入数据
            else if (e.KeyCode == Keys.F8 && e.Alt)
            {
                ...
                if (null != model)
                {
                    if (null != model.Rooms)
                    {
                        ...
                        {
                            AddToGrid("*", "-1", item.RoomNo, item.Price, GetTimeType(item.IsPayByTime), item.PriceHour, item.FreeRoomPriceLimit, item.IsAutoExtendTime, item.Order, GetHideType(item.HideType));
                        }
                    }
                    else
                    {
        ...
        /// 重新加载包厢编号搜索框
        /// </summary>
        private void ReloadRoomNoTextbox(bool TrigChangeEvent = false)
        {
            krptRoomNo.SetValues(null, false);
            if (Resources.GetRes().Rooms.Count > 0)
            {
                krptRoomNo.SetValues(Resources.GetRes().Rooms.OrderByDescending(x => x.Order).ThenBy(x => x.RoomNo.Length).Select(x => x.RoomNo).ToArray(), false);
            }

            if (TrigChangeEvent)
            {
                if (null != ChangeNames)
                    ChangeNames(null, null);
            }
        }
        ...
        /// <returns></returns>
        private int GetTimeTypeNo(string timeType)
        {
            if (timeType == Resources.GetRes().GetString("No"))
                return 0;
            else if (timeType == Resources.GetRes().GetString("Hour"))
                return 1;
            else if (timeType == Resources.GetRes().GetString("Day"))
                return 2;
            else
            {
                throw new Exception(Resources.GetRes().GetString("Exception_InvalidType"));
        ...
        /// 获取时间类型
        /// </summary>
        /// <param name="timeTypeNo"></param>
        /// <returns></returns>
        private string GetTimeType(long timeTypeNo)
        {
            if (timeTypeNo == 0)
                return Resources.GetRes().GetString("No");
            else if (timeTypeNo == 1)
                return Resources.GetRes().GetString("Hour");
            else if (timeTypeNo == 2)
                return Resources.GetRes().GetString("Day");
            else
            {
                throw new Exception(Resources.GetRes().GetString("Exception_InvalidType"));
            }
        ...
        /// 计算价格(为了消准价格)
        /// </summary>
        private void Calc(int rowIndex, int columIndex)
        {
            try
            {
                if (columIndex == 3 || columIndex == 5)
                    krpdgList.Rows[rowIndex].Cells[columIndex].Value = Math.Round(double.Parse(krpdgList.Rows[rowIndex].Cells[columIndex].Value.ToString()), 2);
            }
            catch (Exception ex)
            {
                krpdgList.Rows[rowIndex].Cells[columIndex].Value = "0";


                ExceptionPro.ExpLog(ex);

                MessageBox.Show(string.Format(Resources.GetRes().GetString("LineDataError"), rowIndex + 1), Resources.GetRes().GetString("Warn"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void krpdgList_ColumnSortModeChanged(object sender, DataGridViewColumnEventArgs e)
        {
            ExitEditMode();
        }
