...
{
    internal sealed partial class SupplierPayWindow : KryptonForm
    {
        //页数变量
        private int ListCount = 50;
        private int CurrentPage = 1;
        private int AllPage = 1;
        private List<SupplierPay> resultList = null;
        ...
        public Supplier ReturnValue { get; private set; } //返回值

        public SupplierPayWindow(Supplier Supplier)
        {
            InitializeComponent();
            ...
            if (Resources.GetRes().MainLangIndex == 0)
                name = Supplier.SupplierName0;
            else if (Resources.GetRes().MainLangIndex == 1)
                name = Supplier.SupplierName1;
            else if (Resources.GetRes().MainLangIndex == 2)
                name = Supplier.SupplierName2;

            this.Text = Resources.GetRes().GetString("SupplierPay") + " - " + name;
            ResetPage();
            ...
            this.Icon = new Icon(asm.GetManifestResourceStream(@"Oybab.Res.Resources.Images.PC.SupplierPay.ico"));

            //初始化
            Init();

            // 防止没搜索就增加完时加入时报错
            this.resultList = new List<SupplierPay>();
        ...
        private void Init()
        {
            krpcmSupplierPayId.HeaderText = Resources.GetRes().GetString("Id");
        ...
        private bool IgnoreOperateForSave()
        {
            ...
            if (notHandle)
            {
                var confirm = KryptonMessageBox.Show(this, Resources.GetRes().GetString("IgnoreData"), Resources.GetRes().GetString("SupplierPay"), MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                else
                {
                    ...
                    if (krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*"))
                    {
                        ...
                        // 只有还没保存的才能删除
                        if (krpdgList.SelectedRows[0].Cells["krpcmSupplierPayId"].Value.ToString() == "-1")
        ...
        private void krpdgList_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Control && lastKeyPressed != Keys.EraseEof)
            {
                ...
                if (e.KeyCode == Keys.D)
                {
                    ...
                    // 只有还没保存的才能删除
                    if (krpdgList.SelectedRows.Count > 0 && krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*") && krpdgList.SelectedRows[0].Cells["krpcmSupplierPayId"].Value.ToString() == "-1")
                        Delete();
                }
            }
            else if (e.Control)
        ...
        {
            if (Resources.GetRes().Balances.Where(x => x.HideType == 0 || x.HideType == 3).Count() > 0)
            {
                if (Resources.GetRes().MainLangIndex == 0)
                    krpcmBalanceType.Items.AddRange(Resources.GetRes().Balances.Where(x => x.HideType == 0 || x.HideType == 3).OrderByDescending(x => x.Order).ThenByDescending(x => x.BalanceId).Select(x => x.BalanceName0).ToArray());
                else if (Resources.GetRes().MainLangIndex == 1)
                    krpcmBalanceType.Items.AddRange(Resources.GetRes().Balances.Where(x => x.HideType == 0 || x.HideType == 3).OrderByDescending(x => x.Order).ThenByDescending(x => x.BalanceId).Select(x => x.BalanceName1).ToArray());
                else if (Resources.GetRes().MainLangIndex == 2)
                    krpcmBalanceType.Items.AddRange(Resources.GetRes().Balances.Where(x => x.HideType == 0 || x.HideType == 3).OrderByDescending(x => x.Order).ThenByDescending(x => x.BalanceId).Select(x => x.BalanceName2).ToArray());
            }
