...
{
    internal sealed partial class AdminPayWindow : KryptonForm
    {
        //页数变量
        private int ListCount = 50;
        private int CurrentPage = 1;
        private int AllPage = 1;
        private List<AdminPay> resultList = null;
        private Admin Admin = new Admin();
        private TimeSpan TimeLimit = TimeSpan.FromDays(Resources.GetRes().ShorDay);

        public AdminPayWindow(Admin Admin)
        {
            InitializeComponent();
            ...
            if (Resources.GetRes().MainLangIndex == 0)
                name = Admin.AdminName0;
            else if (Resources.GetRes().MainLangIndex == 1)
                name = Admin.AdminName1;
            else if (Resources.GetRes().MainLangIndex == 2)
                name = Admin.AdminName2;


            this.Text = Resources.GetRes().GetString("AdminPay") + " - " + name;
            ...
            this.Icon = new Icon(asm.GetManifestResourceStream(@"Oybab.Res.Resources.Images.PC.AdminPay.ico"));

            //初始化
            Init();

            // 防止没搜索就增加完时加入时报错
            this.resultList = new List<AdminPay>();
        }
        ...
        private void Init()
        {
            krpcmAdminPayId.HeaderText = Resources.GetRes().GetString("Id");
        ...
        private void krpbSearch_Click(object sender, EventArgs e)
        {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    bool result = OperatesService.GetOperates().ServiceGetAdminPay(Admin.AdminId, -1, addTimeStart, addTimeEnd, -1, out adminPays);
                    this.BeginInvoke(new Action(() =>
                    {
                        if (result)
                        {
                            this.resultList = adminPays.OrderByDescending(x => x.AdminPayId).ToList();
                            //设定页面数据
                            ResetPage();
        ...
        private void OpenPageTo(int pageNo, bool Manual = true)
        {
            ...
            {
                AddToGrid("", item.AdminPayId.ToString(), item.Price, GetBalanceType(item.BalanceId), item.Remark, item.AddTime);
            }
        }
        ...
        private void AddToGrid(string editMark, string Id, double Price, string BalanceName, string Remark, long AddTime)
        {
            string AddTimeStr = "";
            ...
            {
                krpdgList.Rows.Add(editMark, Id, Price.ToString(), BalanceName, Remark, AddTimeStr);
        ...
        private bool IgnoreOperateForSave()
        {
            ...
            if (notHandle)
            {
                var confirm = KryptonMessageBox.Show(this, Resources.GetRes().GetString("IgnoreData"), Resources.GetRes().GetString("AdminPay"), MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void Add()
        {
            ...
            Balance balance = Resources.GetRes().Balances.Where(x => x.HideType == 0 || x.HideType == 3).OrderByDescending(x => x.Order).ThenByDescending(x => x.BalanceId).FirstOrDefault();
            if (null != balance)
                balanceName = GetBalanceType(balance.BalanceId);

            AddToGrid("*", "-1", 0, balanceName, "", long.Parse(DateTime.Now.ToString("yyyyMMddHHmmss")));
        ...
        private void Save()
        {
            ...
            {
                //如果是插入
                if (krpdgList.SelectedRows[0].Cells["krpcmAdminPayId"].Value.ToString().Equals("-1"))
                {
                    AdminPay adminpay = new AdminPay();
                    try
                    {
                        adminpay.AdminPayId = long.Parse(krpdgList.SelectedRows[0].Cells["krpcmAdminPayId"].Value.ToString());
                        adminpay.Price = Math.Round(double.Parse(krpdgList.SelectedRows[0].Cells["krpcmPrice"].Value.ToString()), 2);
                        adminpay.BalanceId = GetBalanceTypeId(krpdgList.SelectedRows[0].Cells["krpcmBalanceType"].Value.ToString());
                        adminpay.Remark = GetValueOrNull(krpdgList.SelectedRows[0].Cells["krpcmRemark"].Value.ToString());
                        adminpay.AdminId = this.Admin.AdminId;
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        {
                            Admin newAdmin;
                            AdminPay newAdminPay;
                            result = OperatesService.GetOperates().ServiceAddAdminPay(Admin, adminpay, out newAdmin, out newAdminPay);
                            this.BeginInvoke(new Action(() =>
                            {
                                if (result.Result)
                                {
        ...
        private void Delete()
        {
            ...
            try
            {
                ...
                Id = long.Parse(krpdgList.SelectedRows[0].Cells["krpcmAdminPayId"].Value.ToString());
            ...
            Task.Factory.StartNew(() =>
            {
                AdminPay adminPay = resultList.Where(x => x.AdminPayId == Id).FirstOrDefault();
                ResultModel result = new ResultModel();
                try
                {
                    Admin newAdmin;
                    ...
                    result = OperatesService.GetOperates().ServiceDelAdminPay(Admin, adminPay, out newAdmin);
                    this.BeginInvoke(new Action(() =>
                    {
                        if (result.Result)
                        {
                            ...
                            AdminPay oldModel = resultList.Where(x => x.AdminPayId == Id).FirstOrDefault();
                            krpdgList.Rows.Remove(krpdgList.SelectedRows[0]);
        ...
        private void krpdgList_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Control && lastKeyPressed != Keys.EraseEof)
            {
                ...
                {
                    // 只有还没保存的才能删除
                    if (krpdgList.SelectedRows.Count > 0 && krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*") && krpdgList.SelectedRows[0].Cells["krpcmAdminPayId"].Value.ToString() == "-1")
                        Delete();
                }
            }
            else if (e.Control)
