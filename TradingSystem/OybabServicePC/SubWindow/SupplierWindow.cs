...
{
    internal sealed partial class SupplierWindow : KryptonForm
    {
        //页数变量
        ...
        public SupplierWindow()
        {
            ...
            Notification.Instance.NotificationSupplier += (obj, value, args) => { this.BeginInvoke(new Action(() => { if (krpdgList.Enabled && resultList.Any(x => x.SupplierId == value.SupplierId)) krpdgList.Enabled = false; })); };

            new CustomTooltip(this.krpdgList);
            this.ControlBox = false;


            krptSupplierNo.Font = new Font(Resources.GetRes().GetString("FontName2"), float.Parse(Resources.GetRes().GetString("FontSize")));
            this.Text = Resources.GetRes().GetString("SupplierManager");
            ResetPage();
            ...
            krplSupplierNo.Text = Resources.GetRes().GetString("SupplierNo");
            ...
            if(int.Parse(Resources.GetRes().GetString("HightFix")) != 0)
            {
                ...
                krptSupplierNo.Location = new Point(krptSupplierNo.Location.X, krptSupplierNo.Location.Y + (int.Parse(Resources.GetRes().GetString("HightFix")) / 2).RecalcMagnification2());
            ...
            // 供应商支付
            LoadContextMenu(kryptonContextMenuItemPay, Resources.GetRes().GetString("SupplierPay"), Resources.GetRes().GetString("SupplierPayDescription"), Image.FromStream(asm.GetManifestResourceStream(@"Oybab.Res.Resources.Images.PC.SupplierPay.png")), (sender, e) => { ShowSupplierPay(); });
            // 供应商到期时间
            LoadContextMenu(kryptonContextMenuItemChangeTime, Resources.GetRes().GetString("ChangeTime"), Resources.GetRes().GetString("SupplierExpiredDescription"), Image.FromStream(asm.GetManifestResourceStream(@"Oybab.Res.Resources.Images.PC.ChangeTime.png")), (sender, e) => { ChangeTime(); });

            this.Icon = new Icon(asm.GetManifestResourceStream(@"Oybab.Res.Resources.Images.PC.Suppliers.ico"));

            //初始化
            Init();

            // 实例化一下, 免得新增时出现问题
            resultList = new List<Supplier>();
        ...
        private void ScanBarcode(string code)
        {
            ...
            //为未保存数据而忽略当前操作
            if (krptSupplierNo.Focused && !IgnoreOperateForSave())
            return;

            if (krptSupplierNo.Focused)
            {
                krptSupplierNo.Text = "";
                StartLoad(this, null);
                ...
                Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        bool result = OperatesService.GetOperates().ServiceGetSupplier(0, null, code.Trim(), null, null, false, out members);
                        this.BeginInvoke(new Action(() =>
                        {
                            if (result)
                            {
                                this.resultList = members.OrderByDescending(x => x.SupplierId).ToList();
                                //设定页面数据
                                ResetPage();
        ...
        private void Init()
        {
            krpcmSupplierId.HeaderText = Resources.GetRes().GetString("Id");
            krpcmSupplierNo.HeaderText = Resources.GetRes().GetString("SupplierNo");
            krpcmCardNo.HeaderText = Resources.GetRes().GetString("CardNo");
            krpcmSupplierName0.HeaderText = string.Format("{0}-{1}", Resources.GetRes().GetString("SupplierName"), Resources.GetRes().GetMainLangByMainLangIndex(0).LangName);
            krpcmSupplierName1.HeaderText = string.Format("{0}-{1}", Resources.GetRes().GetString("SupplierName"), Resources.GetRes().GetMainLangByMainLangIndex(1).LangName);
            krpcmSupplierName2.HeaderText = string.Format("{0}-{1}", Resources.GetRes().GetString("SupplierName"), Resources.GetRes().GetMainLangByMainLangIndex(2).LangName);
        ...
        private void krpbSearch_Click(object sender, EventArgs e)
        {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    bool result = OperatesService.GetOperates().ServiceGetSupplier(0, krptSupplierNo.Text.Trim(), null, null, null, false, out members);
                    this.BeginInvoke(new Action(() =>
                    {
                        if (result)
                        {
        ...
        private void OpenPageTo(int pageNo, bool Manual = true)
        {
            ...
            {
                AddToGrid("", item.SupplierId.ToString(), item.SupplierNo, item.CardNo ?? "", item.SupplierName0, item.SupplierName1, item.SupplierName2, GetSexName(item.Sex), GetOccupationName(item.Occupation), item.Mobile ?? "", item.Phone ?? "", item.IDNumber ?? "", item.Address0 ?? "", item.Address1 ?? "", item.Address2 ?? "", item.OfferRate, item.BalancePrice, item.SpendPrice, item.FavorablePrice, item.IsAllowBorrow, item.ExpiredTime, item.IsEnable, item.Remark ?? "", 0);
            }
        ...
        private void AddToGrid(string editMark, string Id, string memberNo, string cardNo, string memberName0, string memberName1, string memberName2, string sex, string Occupation, string mobile, string phone, string idNumber, string address0, string address1, string address2, double OfferRate, double BalancePrice, double SpendPrice, double FavorablePrice, long IsAllowBorrow, long ExpiredTime, long isEnable, string Remark, int mode)
        {
            ...
            else if (mode == 2)
            {
                ...
                for (int i = 0; i < krpdgList.Rows.Count; i++)
                {
                    ...
                    //只有有改动才可以继续
                    if (krpdgList.Rows[i].Cells["krpcmSupplierId"].Value.ToString() == Id)
                    {
                        rowIndex = i;
                        break;
                    }
        ...
        private bool IgnoreOperateForSave()
        {
            ...
            if (notHandle)
            {
                var confirm = KryptonMessageBox.Show(this, Resources.GetRes().GetString("IgnoreData"), Resources.GetRes().GetString("SupplierManager"), MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (confirm == DialogResult.Yes)
                    return true;
                else
                    return false;
        ...
        private void Add(){
            OpenPageTo(1);
            AddToGrid("*", "-1", "", "", "", "", "", GetSexName(1), GetOccupationName(0), "", "", "", "", "", "", 100, 0, 0, 0, 0,  0, 1, "", 0); // long.Parse(DateTime.Now.ToString("yyyyMMddHHmmss")) to 0
        ...
        private void Save()
        {
            ...
            {
                //如果是插入
                if (krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value.ToString().Equals("-1"))
                {
                    Supplier model = new Supplier();
                    try
                    {
                        // 隐藏功能时先把必要的复制掉(比如语言)
                        Common.GetCommon().CopyForHide(krpdgList.SelectedRows[0].Cells["krpcmSupplierName0"], krpdgList.SelectedRows[0].Cells["krpcmSupplierName1"], krpdgList.SelectedRows[0].Cells["krpcmSupplierName2"], true, false, krpcbMultipleLanguage.Checked);

                        model.SupplierId = long.Parse(krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value.ToString());
                        model.SupplierNo = krpdgList.SelectedRows[0].Cells["krpcmSupplierNo"].Value.ToString().Trim();
                        model.CardNo = GetValueOrNull(krpdgList.SelectedRows[0].Cells["krpcmCardNo"].Value.ToString().Trim());
                        model.SupplierName0 = krpdgList.SelectedRows[0].Cells["krpcmSupplierName0"].Value.ToString().Trim();
                        model.SupplierName1 = krpdgList.SelectedRows[0].Cells["krpcmSupplierName1"].Value.ToString().Trim();
                        model.SupplierName2 = krpdgList.SelectedRows[0].Cells["krpcmSupplierName2"].Value.ToString().Trim();
                        ...
                        //判断空
                        if (string.IsNullOrWhiteSpace(model.SupplierNo) || string.IsNullOrWhiteSpace(model.SupplierName0) || string.IsNullOrWhiteSpace(model.SupplierName1) || string.IsNullOrWhiteSpace(model.SupplierName2))
                        {
                        ...
                        long num;
                        if (!long.TryParse(model.SupplierNo, out num))
                        {
                            KryptonMessageBox.Show(this, string.Format(Resources.GetRes().GetString("LoginValid"), Resources.GetRes().GetString("SupplierNo")), Resources.GetRes().GetString("Warn"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        try
                        {
                            ...
                            bool result = OperatesService.GetOperates().ServiceAddSupplier(model, out IsSupplierExists, out IsCardExists);

                            this.BeginInvoke(new Action(() =>
                            {
                                if (result)
                                {
                                    krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value = model.SupplierId;
                                    krpdgList.SelectedRows[0].Cells["krpcmSupplierNo"].Value = model.SupplierNo;
                                    ...
                                    SetColor(true);
                                    ReloadSupplierTextbox(true);
                                }
                                else
                                {
                                    if (IsSupplierExists)
                                        KryptonMessageBox.Show(this, string.Format(Resources.GetRes().GetString("PropertyExists"), Resources.GetRes().GetString("SupplierNo")), Resources.GetRes().GetString("Warn"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        model = resultList.Where(x => x.SupplierId == model.SupplierId).FirstOrDefault().FastCopy();

                        // 隐藏功能时先把必要的复制掉(比如语言)
                        Common.GetCommon().CopyForHide(krpdgList.SelectedRows[0].Cells["krpcmSupplierName0"], krpdgList.SelectedRows[0].Cells["krpcmSupplierName1"], krpdgList.SelectedRows[0].Cells["krpcmSupplierName2"], false, Ext.AllSame(model.SupplierName0, model.SupplierName1, model.SupplierName2), krpcbMultipleLanguage.Checked);
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        try
                        {
                            ...
                            ResultModel result = OperatesService.GetOperates().ServiceEditSupplier(model, out IsSupplierExists, out IsCardExists);
                            ...
                            this.BeginInvoke(new Action(() =>
                            {
                                if (result.Result)
                                {
                                    KryptonMessageBox.Show(this, Resources.GetRes().GetString("SaveSuccess"), Resources.GetRes().GetString("Information"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value = "";
                                    Supplier oldModel = resultList.Where(x => x.SupplierId == model.SupplierId).FirstOrDefault();
        ...
        /// 显示会员支付详情
        /// </summary>
        private void ShowSupplierPay()
        {
            long Id = -1;
            Id = long.Parse(krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value.ToString());
            SupplierPayWindow pay = new SupplierPayWindow(resultList.Where(x => x.SupplierId == Id).FirstOrDefault());
            pay.StartLoad += (x, y) =>
            {
            ...
            pay.ShowDialog(this);
            Supplier item = pay.ReturnValue;
            if (null != item)
            {
                int no = resultList.FindIndex(x => x.SupplierId == Id);
                resultList.RemoveAt(no);
        ...
        private void Delete()
        {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    ResultModel result = OperatesService.GetOperates().ServiceDelSupplier(resultList.Where(x => x.SupplierId == Id).FirstOrDefault());
                    this.BeginInvoke(new Action(() =>
                    {
                        if (result.Result)
                        {
                            KryptonMessageBox.Show(this, Resources.GetRes().GetString("DeleteSuccess"), Resources.GetRes().GetString("Information"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Supplier oldModel = resultList.Where(x => x.SupplierId == Id).FirstOrDefault();
                            krpdgList.Rows.Remove(krpdgList.SelectedRows[0]);
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                else
                {
                    ...
                    if (krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*"))
                    {
                        ...
                        // 只有还没保存的才能删除
                        if (krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value.ToString() == "-1")
                    ...
                    // 支付记录显示
                    if (krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value.Equals("-1") || krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*"))
                        kryptonContextMenuItemPay.Enabled = false;
                    else
        ...
        private void krpdgList_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Control && lastKeyPressed != Keys.EraseEof)
            {
                ...
                else if (e.KeyCode == Keys.D)
                {
                    ...
                    // 只有还没保存的才能删除
                    if (krpdgList.SelectedRows.Count > 0 && krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*") && krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value.ToString() == "-1")
                        Delete();
                }
                // 选中时才能更改时间
                ...
                // 显示支付记录
                if (e.KeyCode == Keys.E && krpdgList.SelectedRows.Count > 0 && !krpdgList.SelectedRows[0].Cells["krpcmSupplierId"].Value.Equals("-1") && !krpdgList.SelectedRows[0].Cells["krpcmEdit"].Value.Equals("*"))
                    ShowSupplierPay();
        ...
        /// 重新加载产品类型搜索框
        /// </summary>
        private void ReloadSupplierTextbox(bool TrigChangeEvent = false)
        {
        ...
        private void krpcbMultipleLanguage_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                krpcmSupplierName0.Visible = false;
                krpcmSupplierName1.Visible = false;
                krpcmSupplierName2.Visible = false;
                ...
                {
                    krpcmSupplierName0.Visible = true;
                ...
                {
                    krpcmSupplierName1.Visible = true;
                ...
                {
                    krpcmSupplierName2.Visible = true;
