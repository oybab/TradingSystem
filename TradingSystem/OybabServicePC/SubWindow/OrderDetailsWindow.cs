...
{
    internal sealed partial class OrderDetailsWindow : KryptonForm
    {
        //页数变量
        ...
        public OrderDetailsWindow(Order order, List<OrderDetail> orderdetails, List<OrderPay> payList)
        {
            this.order = order;
            this.resultList = orderdetails;
        ...
        private void OpenPageTo(int pageNo, bool Manual = true)
        {
            ...
            {
                AddToGrid("", item.OrderDetailId.ToString(), item.ProductId, item.Price, item.Count, item.TotalPrice, item.IsPack, GetOrderDetailsState(item.State), item.Request, item.AddTime, item.AdminId);
            }
        ...
        {
            //获取数据
            var currentResult = payList.OrderByDescending(x => x.OrderPayId);
            //添加到数据集中
            krpdgPayList.Rows.Clear();
            foreach (var item in currentResult)
            {
                AddToGrid2("", item.OrderPayId.ToString(), GetOrderDetailsState(item.State), item.BalanceId, item.tb_member, item.OriginalPrice, item.RemovePrice, item.BalancePrice, item.AddTime, item.AdminId);
            }
        ...
        private void AddToGrid(string editMark, string Id, long productId, double Price, double Count, double TotalPrice, long IsPack, string State, string Request, long AddTime, long AdminId)
        {
            string productName = "";
            string AddTimeStr = "";
            string RequestNames = "";
        ...
        private void CheckHistory()
        {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    bool result = OperatesService.GetOperates().ServiceGetLog(-1, -1, startTimeFinal, endTimeFinal, out Balances, out logList, this.order.OrderId);
                    this.BeginInvoke(new Action(() =>
                    {
                        if (result)
                        {
